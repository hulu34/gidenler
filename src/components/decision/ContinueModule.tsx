          ...
          return (
            <li key={r.entityId} className="grid gap-x-8 gap-y-2 border-b border-line py-5 sm:grid-cols-[1fr_auto]">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[13px] text-ink-2">{r.state === "want_to_go" ? "Gitmek istiyorum demiştin." : "Gittiğini söyledin — nasıl geçti?"}{it.overallScore !== null && <span className="tnum text-ink-3"> · Gidenler {score1(it.overallScore)}{it.scoreTrend.direction === "up" ? " ↑" : it.scoreTrend.direction === "down" ? " ↓" : ""}</span>}</span>
              </div>
