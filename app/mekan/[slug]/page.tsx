    ...
    <div className="mx-auto max-w-[1180px] px-5 pb-32 sm:px-7 sm:pb-24">
      ...
      <header className="flex flex-col gap-4 pt-8 sm:pt-12">
        ...
        <p className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[13px] text-ink-2">
          {[entity.location?.neighborhood && entity.location.neighborhood !== district ? `${entity.location.neighborhood}${entity.location.city !== "İstanbul" ? ` · ${entity.location.city}` : ""}` : entity.location?.city !== "İstanbul" ? entity.location?.city : null, entity.address, entity.hours, PRICE(entity.priceLevel)].filter(Boolean).map((x, i) => (
            <span key={i} className="flex items-center gap-3">{i > 0 && <span aria-hidden className="text-ink-3">·</span>}<span className={i === 3 ? "tnum" : ""}>{x}</span></span>
          ))}
          {entity.isDemo && <Badge tone="demo" className="ml-1">Demo kayıt</Badge>}
        </p>

        {blurbs[entity.id] && (
          <p className="prose-exp max-w-[60ch] text-[14.5px] leading-[1.5] text-ink-2 sm:text-[15px]">{blurbs[entity.id]}</p>
        )}
      ...
      {/* ═══════════ KATMAN 1 — KARAR: puan · yön · güven · sana göre · hüküm · eylem ═══════════ */}
      {I.overallScore !== null ? (
        <section className="mt-7 border-t-2 border-line-strong pt-6" aria-labelledby="karar">
          <h2 id="karar" className="sr-only">Karar</h2>
          <ScoreBlock intel={I} />
          {c.mode === "standard" && (
            <div className="mt-7">
              <DecisionHero
      ...
      {/* ═══════════ KATMAN 2 — KANIT: Neye dayanıyor? (kompakt, editoryal) ═══════════ */}
      {I.overallScore !== null && (
        <section className="mt-12 border-t-2 border-line-strong pt-6" aria-labelledby="kanit">
          <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
            <h2 id="kanit" className="text-[13px] font-bold uppercase tracking-[0.2em]">Neye dayanıyor?</h2>
            <p className="max-w-[48ch] text-[12px] text-ink-3">Puanın arkasındaki kanıt. Renk kaliteyi, ok yönü söyler.</p>
          </div>
          <div className="mt-5"><EvidenceStrip intel={I} returnLabel={schema.returnLabel} lastVisitedAt={experiences[0]?.visitedAt ?? (I.timeline.length ? `${I.timeline[I.timeline.length - 1].period}-20` : undefined)} /></div>
      ...
      {I.overallScore !== null && (
        <section className="mt-12 border-t-2 border-line-strong pt-6" aria-labelledby="derin">
          <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
            ...
            <p className="text-[12px] text-ink-3">Karar için gerekmez; merak eden iner. Hiçbiri kararı değiştirmez, hepsi açıklar.</p>
          </div>

          <Disclosure title="Deneyim özeti ve konular" hint={[I.aiSummary ? `${nf(I.aiSummary.basedOnCount)} deneyimin özeti` : null, I.positiveThemes[0] ? `en çok: ${I.positiveThemes[0].label.toLocaleLowerCase("tr")}` : null, I.negativeThemes[0] ? `şikâyet: ${I.negativeThemes[0].label.toLocaleLowerCase("tr")}` : null].filter(Boolean).join(" · ")}>
          ...
          {I.timeline.length > 1 && (
            <Disclosure title="Gidenler Trend" hint={`${I.timeline.length} dönem · son 90 gün ${I.scoreTrend.delta > 0 ? "+" : ""}${score1(I.scoreTrend.delta)} · ${I.momentum === "up" || I.momentum === "strong_up" ? "yükseliyor" : I.momentum === "down" || I.momentum === "strong_down" ? "geriliyor" : "stabil"}`}>
              <TrendModule
          ...
          {experiences.length > 0 ? (
            ...
            <p className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 border-b border-line py-4 text-[13px] text-ink-2">
              <span>Bu kayıt için deneyim metinleri prototipte gösterilmiyor; puan ve yön {nf(I.experienceCount)} deneyimin özetidir.</span>
      ...
      {I.overallScore === null && (
        <section className="mt-10" aria-labelledby="deneyimler">
          ...
          <div className="mt-7">
            {experiences.length ? experiences.map((e) => (
              <ExperienceCard key={e.id} experience={e} schema={schema} showScores={c.showScores} entitySlug={entity.slug} />
            )) : (
              <p className="text-[13.5px] text-ink-2">Bu kayıt için deneyim metni prototipte gösterilmiyor; yalnızca nötr konu sayımı ({nf(I.experienceCount)} deneyim) var. Bu kategoride puan, özet ve öneri üretilmez.</p>
            )}
          </div>
        </section>
      )}
