import { universeStats } from "@/data/universe";
import type {
 ...
 */
const baseSimilar: Record<string, SimilarUsersPerspective> = {
 ...
 */
const baseContextFit: Record<string, Partial<Record<DecisionContextKey, number>>> = {
...
const baseAmbient: Record<string, { quiet: number; speed: number }> = {
...
/* Evren: sentetik kayıtların benzer-kullanıcı, bağlam ve ortam sinyalleri (deterministik). */
const uSimilar: Record<string, SimilarUsersPerspective> = {};
const uFit: Record<string, Partial<Record<DecisionContextKey, number>>> = {};
const uAmbient: Record<string, { quiet: number; speed: number }> = {};
for (const [id, st] of Object.entries(universeStats)) {
  if (st.similar) uSimilar[id] = { entityId: id, score: st.similar.score, sampleSize: st.similar.sampleSize, returnRate: st.similar.returnRate, confidence: st.similar.sampleSize >= 60 ? "high" : st.similar.sampleSize >= 20 ? "medium" : "low", isDemo: true };
  uFit[id] = st.contextFit as Partial<Record<DecisionContextKey, number>>;
  uAmbient[id] = { quiet: st.quiet, speed: st.speed };
}
export const similarUsers: Record<string, SimilarUsersPerspective> = { ...uSimilar, ...baseSimilar };
export const contextFit: Record<string, Partial<Record<DecisionContextKey, number>>> = { ...uFit, ...baseContextFit };
export const ambientSignals: Record<string, { quiet: number; speed: number }> = { ...uAmbient, ...baseAmbient };
