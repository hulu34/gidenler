...
export function getPersonalMatch(
  ...
  const noun = entity.categoryId === "cat.film" ? "film" : entity.categoryId === "cat.show" ? "etkinlik" : entity.categoryId === "cat.hotel" ? "otel" : "mekân";
  ...
  if (quietW >= 60) {
    factors.push({
      ...
      evidence: ambient.quiet >= 7 ? `Deneyimlerde sakin bir ${noun} olarak anlatılıyor` : ambient.quiet <= 4 ? "Deneyimlerde gürültü ve kalabalık sık geçiyor" : "Gürültü konusunda deneyimler ikiye bölünmüş",
    });
  }
  ...
  if (cuisineLabel) {
    factors.push({
      ...
      evidence: cuisine > 0 ? `${cuisineLabel} ${noun === "mekân" ? "mekânlarını" : `${noun}lerini`} ${cuisineLevel} değerlendiriyorsun` : `${cuisineLabel} senin listende ${cuisineLevel}`,
    });
  }
  ...
  /* Sentetik kayıtlarda kohort el yapımı değil: uyum 92'yi aşmaz (kanıt sınırı). */
  const raw = round(clamp(base + cuisine + simEffect + trendEffect + valueTrend + price + fit));
  const cap = !entity.isSynthetic ? 100 : entity.tier === "A" ? 92 : entity.tier === "B" ? 89 : 85;
  const score = entity.isSynthetic && entity.tier === "C" ? Math.min(cap, round(raw * 0.93)) : Math.min(cap, raw);
...
/** Hüküm metni kategoriye göre: "Kesinlikle gidilir" → film için "Kesinlikle izlenir", otel için "Kesinlikle kalınır". */
export function verdictText(v: DecisionVerdict, V: { want: string }): string {
  const stem = V.want.split(" ")[0]; /* Gitmek / İzlemek / Kalmak / Katılmak */
  const pass = stem === "Gitmek" ? "gidilir" : stem === "İzlemek" ? "izlenir" : stem === "Kalmak" ? "kalınır" : "katılınır";
  if (v === "Kesinlikle gidilir") return `Kesinlikle ${pass}`;
  if (v === "Gidilir") return pass.charAt(0).toLocaleUpperCase("tr") + pass.slice(1);
  return v;
}
...
export function getDecision(entityId: string, context: DecisionContextKey = "default", overrides?: Partial<Record<string, number>>, profile?: TasteProfile): Decision | null {
  ...
  if (it.returnRate >= 0.8) reasons.push({ text: `Gidenlerin ${pctOf(Math.round(it.returnRate * 100))} ${verbsFor(entity).again.toLocaleLowerCase("tr")} diyor`, kind: "community", source: `${nf(it.experienceCount)} deneyim` });
  ...
  return {
    entityId, context, verdict: verdictOf(m.score, it), verdictText: verdictText(verdictOf(m.score, it), verbsFor(entity)), personalMatch: m.score,
...
const DISTRICTS: Array<[RegExp, string]> = [
  [/kad[ıi]k[öo]y|moda\b|caddebostan|suadiye|ko[şs]uyolu|yelde[ğg]irmeni/i, "Kadıköy"], [/ni[şs]anta[şs][ıi]|[şs]i[şs]li|te[şs]vikiye|bomonti/i, "Şişli"],
  [/be[şs]ikta[şs]|akaretler|bebek|etiler|ortak[öo]y|arnavutk[öo]y/i, "Beşiktaş"], [/beyo[ğg]lu|cihangir|karak[öo]y|taksim|galata/i, "Beyoğlu"],
  [/[üu]sk[üu]dar|kuzguncuk|[çc]engelk[öo]y/i, "Üsküdar"], [/sar[ıi]yer|emirgan|tarabya|maslak|yenik[öo]y/i, "Sarıyer"], [/fatih|balat|fener|sultanahmet/i, "Fatih"],
  [/bak[ıi]rk[öo]y|ye[şs]ilk[öo]y|florya/i, "Bakırköy"], [/beykoz|kanl[ıi]ca/i, "Beykoz"], [/adalar|b[üu]y[üu]kada|heybeliada/i, "Adalar"],
];
/** Şehir: İstanbul dışı bir şehir geçiyorsa aday havuzu o şehre kısıtlanır. */
const CITY_RE: Array<[RegExp, string]> = [
  [/ankara|tunal[ıi]|k[ıi]z[ıi]lay/i, "Ankara"], [/[iİ]zmir|alsancak|ala[çc]at[ıi]|[çc]e[şs]me|urla/i, "İzmir"], [/bursa|mudanya/i, "Bursa"], [/antalya|kalei[çc]i|ka[şs]\b|kalkan/i, "Antalya"],
  [/adana/i, "Adana"], [/gaziantep|antep/i, "Gaziantep"], [/eski[şs]ehir|odunpazar[ıi]/i, "Eskişehir"], [/mu[ğg]la|bodrum|fethiye|dat[çc]a|marmaris/i, "Muğla"], [/trabzon/i, "Trabzon"], [/mersin/i, "Mersin"], [/hatay|antakya/i, "Hatay"],
];
const FACETS: Array<[RegExp, string]> = [
  ...
  [/kahvalt[ıi]|serpme|boyoz/i, "Kahvaltı"], [/f[ıi]r[ıi]n|ekmek/i, "Fırın ve ekmek"], [/esnaf|lokanta|ev yeme[ğg]i/i, "Esnaf lokantası"],
  [/fine dining|[şs]ef/i, "Fine dining"], [/manzara|teras/i, "Manzara"],
];

export function parseAsk(text: string): AskQuery {
  const t = text.trim();
  const tl0 = t.toLocaleLowerCase("tr");
  const q: AskQuery = { text: t, context: "default", priorities: [] };
  for (const [re, d] of DISTRICTS) if (re.test(tl0)) { q.district = d; break; }
  for (const [re, c] of CITY_RE) if (re.test(tl0)) { q.city = c; break; }
...
export function askGidenler(text: string, refine: AskRefine = {}): AskResult {
  ...
  if (q.district) understood.push(q.district); else if (q.city) understood.push(q.city);
  ...
  /* "X mı Y mı?" — yalnızca kanıtlı (A) kayıtlar; yer adları (Kadıköy, Moda…) mekân adı sayılmaz. */
  const lt = lower(text);
  const placeWords = new Set(entities.flatMap((e) => [e.location?.district, e.location?.neighborhood, e.location?.city]).filter(Boolean).map((x) => lower(x as string)));
  const named = entities.filter((e) => {
    if (e.tier !== "A") return false;
    if (lt.includes(lower(e.name))) return true;
    const first = lower(e.name.split(" ")[0]);
    return first.length >= 5 && !placeWords.has(first) && lt.includes(first);
  });

  /* Öneri havuzu: karar verilebilecek kadar kanıtı olan (A katmanı) yeme-içme mekânları. Keşif kayıtları arama/keşfette görünür, öneride değil. */
  let pool = listCards().filter((c) => c.score !== null && c.category.compliance.showScores && c.entity.tier === "A" && ["cat.restaurant", "cat.cafe", "cat.bar"].includes(c.category.id));
  ...
  if (named.length >= 2) {
    ...
    if (q.district) {
      /* Şehir sert kısıt, semt yumuşak: aynı şehirde semt dışı adaylar cezayla kalır ve açıkça etiketlenir — çok daha iyi bir seçenek varsa görünür. */
      const city = pool.find((c) => c.entity.location?.district === q.district)?.entity.location?.city;
      if (city) pool = pool.filter((c) => c.entity.location?.city === city);
      for (const c of pool) if (c.entity.location?.district !== q.district) outside.add(c.entity.id);
    } else if (q.city) {
      const inCity = pool.filter((c) => c.entity.location?.city === q.city); if (inCity.length) pool = inCity;
    } else if (!refine.side) {
      /* Şehir belirtilmediyse varsayılan şehir İstanbul; diğer şehirler yalnızca adıyla sorulduğunda gelir. */
      pool = pool.filter((c) => c.entity.location?.city === "İstanbul");
    }
  }
  ...
  const items: AskResultItem[] = scored.map(({ c, m, score }) => {
    ...
    /* Bu soru için uyum: bağlam (sessizlik, bütçe, semt) uygulanmış değer — mekân sayfasındaki genel uyumdan farklı olabilir, Date/Aile gibi. */
    return { entityId: c.entity.id, match: Math.min(99, m!.score + 6, round(score)), reasons: reasons.slice(0, 4), warning };
  });
...
export function gidenlerNow(context: DecisionContextKey = "default") {
  const evening = listCards().filter((c) => c.score !== null && c.entity.tier !== "C" && ["cat.restaurant", "cat.cafe", "cat.bar"].includes(c.category.id) && closesAfter(c.entity.hours, 21));
  return evening
...
export function forYou(limit = 3) {
  return listCards()
    .filter((c) => c.score !== null && c.entity.tier === "A")
...
export function getNotifications(relationships: Record<string, UserEntityRelationship>): NotificationEvent[] {
  ...
  for (const rel of Object.values(relationships)) {
    ...
    if (!e || !it) continue;
    const V = verbsFor(e); const wantN = `${V.want.split(" ")[0]} istediğin`; /* "Gitmek istediğin" / "İzlemek istediğin" */
    if (rel.state === "want_to_go") {
      if (it.momentum === "up" || it.momentum === "strong_up") out.push({ id: `n.up.${e.id}`, kind: "want_to_go_rising", entityId: e.id, title: `${wantN} ${e.name} son 30 günde yükseliyor`, body: `Gidenler ${score1(it.overallScore ?? 0)} · ${it.scoreTrend.delta > 0 ? "+" : ""}${score1(it.scoreTrend.delta)} son 90 gün`, at: "2026-09-02", basedOn: "want_to_go" });
      const dimUp = it.ratingDimensions.find((d) => d.trend.direction === "up" && d.trend.delta >= 0.2);
      if (dimUp) out.push({ id: `n.dim.${e.id}`, kind: "want_to_go_rising", entityId: e.id, title: `${wantN} ${e.name}'da ${lower(dimUp.label)} deneyimleri toparlanıyor`, body: `${dimUp.label} ${score1(dimUp.value)} · +${score1(dimUp.trend.delta)} son 90 gün`, at: "2026-09-01", basedOn: "want_to_go" });
      out.push({ id: `n.went.${e.id}`, kind: "went_yet", entityId: e.id, title: `${e.name} — ${V.didQ.toLocaleLowerCase("tr")}`, body: `${V.want} demiştin. Nasıl geçtiğini iki dokunuşla söyle.`, at: "2026-09-02", basedOn: "want_to_go" });
    }
    if (rel.state === "saved") {
