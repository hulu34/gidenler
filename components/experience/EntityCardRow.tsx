      ...
      >
        <div className="flex min-w-0 flex-col gap-1.5">
          ...
          <span className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-3">
            <span className="text-accent-ink">{entity.subcategory ?? category.label}</span>
            {entity.location?.district && <span>{entity.location.city !== "İstanbul" ? `${entity.location.city} · ` : ""}{entity.location.district}{entity.location.neighborhood ? ` · ${entity.location.neighborhood}` : ""}</span>}
