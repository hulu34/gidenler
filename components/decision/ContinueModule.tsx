          ...
          return (
            <li key={r.entityId} className="grid gap-x-8 gap-y-2 border-b border-line py-5 sm:grid-cols-[1fr_auto]">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[13px] text-ink-2">{r.state === "want_to_go" ? `${verbsFor(e).want} demiştin.` : `${verbsFor(e).didYou} — nasıl geçti?`}{it.overallScore !== null && <span className="ml-2 inline-flex"><ScoreNumber score={it.overallScore} size="sm" trend={{ direction: it.scoreTrend.direction, delta: it.scoreTrend.delta }} /></span>}</span>
              </div>
